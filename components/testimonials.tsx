"use client";

import { useState, useEffect, useCallback } from "react";
import Image, { StaticImageData } from "next/image";
import TestimonialImg01 from "@/public/images/testimonial-01.jpg";
import TestimonialImg02 from "@/public/images/testimonial-02.jpg";
import TestimonialImg03 from "@/public/images/testimonial-03.jpg";
import TestimonialImg04 from "@/public/images/testimonial-04.jpg";
import TestimonialImg05 from "@/public/images/testimonial-05.jpg";
import TestimonialImg06 from "@/public/images/testimonial-06.jpg";
import TestimonialImg07 from "@/public/images/testimonial-07.jpg";
import TestimonialImg08 from "@/public/images/testimonial-08.jpg";
import TestimonialImg09 from "@/public/images/testimonial-09.jpg";
import ClientImg01 from "@/public/images/client-logo-01.svg";
import ClientImg02 from "@/public/images/client-logo-02.svg";
import ClientImg03 from "@/public/images/client-logo-03.svg";
import ClientImg04 from "@/public/images/client-logo-04.svg";
import ClientImg05 from "@/public/images/client-logo-05.svg";
import ClientImg06 from "@/public/images/client-logo-06.svg";
import ClientImg07 from "@/public/images/client-logo-07.svg";
import ClientImg08 from "@/public/images/client-logo-08.svg";
import ClientImg09 from "@/public/images/client-logo-09.svg";

const testimonials = [
  {
    img: TestimonialImg01,
    clientImg: ClientImg01,
    name: "Kushi Educational Service",
    company: "Education",
    content:
      "Complete Instagram handling, Meta ads, lead generation, creative posters, and AI videos.Successfully improved brand visibility and generated quality leads.Ongoing collaboration based on consistent performance and client satisfaction.",
    categories: [1, 3, 5],
  },
  {
    img: TestimonialImg02,
    clientImg: ClientImg02,
    name: "Skillon English Academy",
    company: "Education",
    content:
      "Complete digital marketing solution: Website, SEO, Meta ads, creative design, and social media management.Built strong online presence and consistent student lead generation through strategic branding and performance marketing.",
    categories: [1, 2, 4],
  },
  {
    img: TestimonialImg03,
    clientImg: ClientImg03,
    name: "Dream Fashion Academy",
    company: "Fashion",
    content:
      "Creative branding, Meta ads campaigns, and Google My Business optimization.Enhanced local visibility, improved lead generation, and strengthened brand presence.Client satisfied with consistent performance and growth.",
    categories: [1, 2, 5],
  },
  {
    img: TestimonialImg04,
    clientImg: ClientImg04,
    name: "Shubamasthu Cafe",
    company: "Food & Beverage",
    content:
      "Creative posters, AI promotional videos, and Instagram handling.Enhanced brand aesthetics and improved social media engagement through strategic visual content.",
    categories: [1, 4],
  },
];

export default function Testimonials() {
  const [category, setCategory] = useState<number>(1);
  const [currentIndex, setCurrentIndex] = useState(0);
  const [isAutoPlaying, setIsAutoPlaying] = useState(true);

  const filteredTestimonials = testimonials.filter((testimonial) =>
    testimonial.categories.includes(category)
  );

  const nextSlide = useCallback(() => {
    setCurrentIndex((prevIndex) =>
      prevIndex === filteredTestimonials.length - 1 ? 0 : prevIndex + 1
    );
  }, [filteredTestimonials.length]);

  const prevSlide = useCallback(() => {
    setCurrentIndex((prevIndex) =>
      prevIndex === 0 ? filteredTestimonials.length - 1 : prevIndex - 1
    );
  }, [filteredTestimonials.length]);

  useEffect(() => {
    let interval: NodeJS.Timeout;
    if (isAutoPlaying && filteredTestimonials.length > 0) {
      interval = setInterval(() => {
        nextSlide();
      }, 5000);
    }
    return () => clearInterval(interval);
  }, [isAutoPlaying, nextSlide, filteredTestimonials.length]);

  useEffect(() => {
    setCurrentIndex(0);
  }, [category]);

  if (filteredTestimonials.length === 0) {
    return (
      <section id="work" className="scroll-mt-24">
        <div className="mx-auto max-w-6xl px-4 sm:px-6">
          <div className="border-t py-12 [border-image:linear-gradient(to_right,transparent,#2563eb1a,transparent)1] md:py-20">
            <div className="mx-auto max-w-3xl pb-12 text-center">
              <h2 className="animate-[gradient_6s_linear_infinite] bg-[linear-gradient(to_right,var(--color-gray-200),#60a5fa,var(--color-gray-50),#3b82f6,var(--color-gray-200))] bg-[length:200%_auto] bg-clip-text pb-4 font-nacelle text-3xl font-semibold text-transparent md:text-4xl">
               Proven Results
              </h2>
              <p className="text-lg text-blue-200">
                We partner with ambitious brands across education, fashion, and local businesses to deliver measurable digital growth.
From strategic campaigns to creative execution, our work reflects performance, precision, and long-term brand impact.
              </p>
            </div>
            <div className="text-center text-blue-200">
              No testimonials found for this category.
            </div>
          </div>
        </div>
      </section>
    );
  }

  return (
    <section id="work" className="scroll-mt-24">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="border-t py-12 [border-image:linear-gradient(to_right,transparent,#2563eb1a,transparent)1] md:py-20">
          {/* Section header */}
          <div className="mx-auto max-w-3xl pb-12 text-center">
            <h2 className="animate-[gradient_6s_linear_infinite] bg-[linear-gradient(to_right,var(--color-gray-200),#60a5fa,var(--color-gray-50),#3b82f6,var(--color-gray-200))] bg-[length:200%_auto] bg-clip-text pb-4 font-nacelle text-3xl font-semibold text-transparent md:text-4xl">
              Proven Results
            </h2>
            <p className="text-lg text-blue-200">
             We partner with ambitious brands across education, fashion, and local businesses to deliver measurable digital growth.
From strategic campaigns to creative execution, our work reflects performance, precision, and long-term brand impact.
            </p>
          </div>

          <div>
            {/* Buttons */}
            <div className="flex justify-center pb-12 max-md:hidden md:pb-16">
              <div className="relative inline-flex flex-wrap justify-center rounded-[1.25rem] bg-blue-950/40 p-1 backdrop-blur-xs">
                {/* Button #1 */}
                <button
                  className={`flex h-8 flex-1 items-center gap-2.5 whitespace-nowrap rounded-full px-3 text-sm font-medium text-white transition-colors focus-visible:outline-hidden focus-visible:ring-3 focus-visible:ring-blue-400 ${
                    category === 1
                      ? "relative bg-gradient-to-b from-blue-600 via-blue-500 to-blue-600 before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border before:border-transparent before:[background:linear-gradient(to_bottom,#3b82f600,#3b82f680)_border-box] before:[mask-composite:exclude_!important] before:[mask:linear-gradient(white_0_0)_padding-box,_linear-gradient(white_0_0)]"
                      : "bg-blue-950/60 text-blue-100 opacity-80 transition-opacity hover:bg-blue-900/60 hover:opacity-100"
                  }`}
                  aria-pressed={category === 1}
                  onClick={() => setCategory(1)}
                >
                  <svg
                    className={`fill-current ${
                      category === 1 ? "text-white" : "text-blue-400"
                    }`}
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height={16}
                  >
                    <path d="M.062 10.003a1 1 0 0 1 1.947.455c-.019.08.01.152.078.19l5.83 3.333c.052.03.115.03.168 0l5.83-3.333a.163.163 0 0 0 .078-.188 1 1 0 0 1 1.947-.459 2.161 2.161 0 0 1-1.032 2.384l-5.83 3.331a2.168 2.168 0 0 1-2.154 0l-5.83-3.331a2.162 2.162 0 0 1-1.032-2.382Zm7.854-7.981-5.83 3.332a.17.17 0 0 0 0 .295l5.828 3.33c.054.031.118.031.17.002l5.83-3.333a.17.17 0 0 0 0-.294L8.085 2.023a.172.172 0 0 0-.17-.001ZM9.076.285l5.83 3.332c1.458.833 1.458 2.935 0 3.768l-5.83 3.333c-.667.38-1.485.38-2.153-.001l-5.83-3.332c-1.457-.833-1.457-2.935 0-3.767L6.925.285a2.173 2.173 0 0 1 2.15 0Z" />
                  </svg>
                  <span>View All</span>
                </button>
                {/* Button #2 */}
                <button
                  className={`flex h-8 flex-1 items-center gap-2.5 whitespace-nowrap rounded-full px-3 text-sm font-medium text-white transition-colors focus-visible:outline-hidden focus-visible:ring-3 focus-visible:ring-blue-400 ${
                    category === 2
                      ? "relative bg-gradient-to-b from-blue-600 via-blue-500 to-blue-600 before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border before:border-transparent before:[background:linear-gradient(to_bottom,#3b82f600,#3b82f680)_border-box] before:[mask-composite:exclude_!important] before:[mask:linear-gradient(white_0_0)_padding-box,_linear-gradient(white_0_0)]"
                      : "bg-blue-950/60 text-blue-100 opacity-80 transition-opacity hover:bg-blue-900/60 hover:opacity-100"
                  }`}
                  aria-pressed={category === 2}
                  onClick={() => setCategory(2)}
                >
                  <svg
                    className={`fill-current ${
                      category === 2 ? "text-white" : "text-blue-400"
                    }`}
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height={16}
                  >
                    <path d="M6.5 3.5a1.5 1.5 0 1 1 3 0 1.5 1.5 0 0 1-3 0ZM9 6.855A3.502 3.502 0 0 0 8 0a3.5 3.5 0 0 0-1 6.855v1.656L5.534 9.65a3.5 3.5 0 1 0 1.229 1.578L8 10.267l1.238.962a3.5 3.5 0 1 0 1.229-1.578L9 8.511V6.855Zm2.303 4.74c.005-.005.01-.01.013-.016l.012-.016a1.5 1.5 0 1 1-.025.032ZM3.5 11A1.497 1.497 0 0 1 5 12.5 1.5 1.5 0 1 1 3.5 11Z" />
                  </svg>
                  <span>Web Apps</span>
                </button>
                {/* Button #3 */}
                <button
                  className={`flex h-8 flex-1 items-center gap-2.5 whitespace-nowrap rounded-full px-3 text-sm font-medium text-white transition-colors focus-visible:outline-hidden focus-visible:ring-3 focus-visible:ring-blue-400 ${
                    category === 3
                      ? "relative bg-gradient-to-b from-blue-600 via-blue-500 to-blue-600 before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border before:border-transparent before:[background:linear-gradient(to_bottom,#3b82f600,#3b82f680)_border-box] before:[mask-composite:exclude_!important] before:[mask:linear-gradient(white_0_0)_padding-box,_linear-gradient(white_0_0)]"
                      : "bg-blue-950/60 text-blue-100 opacity-80 transition-opacity hover:bg-blue-900/60 hover:opacity-100"
                  }`}
                  aria-pressed={category === 3}
                  onClick={() => setCategory(3)}
                >
                  <svg
                    className={`fill-current ${
                      category === 3 ? "text-white" : "text-blue-400"
                    }`}
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height={16}
                  >
                    <path d="M2.428 10c.665-1.815 1.98-3.604 3.44-4.802-.6-1.807-1.443-3.079-2.29-3.18-1.91-.227-2.246 2.04-.174 2.962a1 1 0 1 1-.813 1.827C-1.407 5.028-.589-.491 3.815.032c1.605.191 2.925 1.811 3.79 4.07.979-.427 1.937-.51 2.735-.092.818.429 1.143 1.123 1.294 2.148.015.1.022.149.043.32.542-.537 1.003-.797 1.693-.622.64.162.894.493 1.195 1.147l.018.04a1 1 0 0 1 1.133 1.61c-.46.47-1.12.574-1.744.398a1.661 1.661 0 0 1-.87-.592 2.127 2.127 0 0 1-.224-.349 3.225 3.225 0 0 1-.55.477c-.377.253-.8.368-1.259.267-.993-.218-1.21-.779-1.367-2.05-.027-.22-.033-.262-.046-.353-.067-.452-.144-.617-.244-.67-.225-.118-.665-.013-1.206.278.297 1.243.475 2.587.516 3.941H15a1 1 0 0 1 0 2H8.68l-.025.285c-.173 1.918-.906 3.381-2.654 3.668-1.5.246-3.013-.47-3.677-1.858-.29-.637-.39-1.35-.342-2.095H1a1 1 0 0 1 0-2h1.428Zm2.11 0h2.175a18.602 18.602 0 0 0-.284-2.577c-.205.202-.408.42-.606.654A9.596 9.596 0 0 0 4.537 10Zm2.135 2H3.942c-.032.465.03.888.194 1.25.258.538.89.836 1.54.73.546-.09.888-.772.988-1.875L6.673 12Z" />
                  </svg>
                  <span>eCommerce</span>
                </button>
                {/* Button #4 */}
                <button
                  className={`flex h-8 flex-1 items-center gap-2.5 whitespace-nowrap rounded-full px-3 text-sm font-medium text-white transition-colors focus-visible:outline-hidden focus-visible:ring-3 focus-visible:ring-blue-400 ${
                    category === 4
                      ? "relative bg-gradient-to-b from-blue-600 via-blue-500 to-blue-600 before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border before:border-transparent before:[background:linear-gradient(to_bottom,#3b82f600,#3b82f680)_border-box] before:[mask-composite:exclude_!important] before:[mask:linear-gradient(white_0_0)_padding-box,_linear-gradient(white_0_0)]"
                      : "bg-blue-950/60 text-blue-100 opacity-80 transition-opacity hover:bg-blue-900/60 hover:opacity-100"
                  }`}
                  aria-pressed={category === 4}
                  onClick={() => setCategory(4)}
                >
                  <svg
                    className={`fill-current ${
                      category === 4 ? "text-white" : "text-blue-400"
                    }`}
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height={16}
                  >
                    <path d="M3.757 3.758a6 6 0 0 1 8.485 8.485 5.992 5.992 0 0 1-5.301 1.664 1 1 0 1 0-.351 1.969 8 8 0 1 0-4.247-2.218 1 1 0 0 0 1.415-.001L9.12 8.294v1.827a1 1 0 1 0 2 0v-4.2a.997.997 0 0 0-1-1.042H5.879a1 1 0 1 0 0 2h1.829l-4.599 4.598a6 6 0 0 1 .648-7.719Z" />
                  </svg>
                  <span>Enterprise</span>
                </button>
                {/* Button #5 */}
                <button
                  className={`flex h-8 flex-1 items-center gap-2.5 whitespace-nowrap rounded-full px-3 text-sm font-medium text-white transition-colors focus-visible:outline-hidden focus-visible:ring-3 focus-visible:ring-blue-400 ${
                    category === 5
                      ? "relative bg-gradient-to-b from-blue-600 via-blue-500 to-blue-600 before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border before:border-transparent before:[background:linear-gradient(to_bottom,#3b82f600,#3b82f680)_border-box] before:[mask-composite:exclude_!important] before:[mask:linear-gradient(white_0_0)_padding-box,_linear-gradient(white_0_0)]"
                      : "bg-blue-950/60 text-blue-100 opacity-80 transition-opacity hover:bg-blue-900/60 hover:opacity-100"
                  }`}
                  aria-pressed={category === 5}
                  onClick={() => setCategory(5)}
                >
                  <svg
                    className={`fill-current ${
                      category === 5 ? "text-white" : "text-blue-400"
                    }`}
                    xmlns="http://www.w3.org/2000/svg"
                    width="16"
                    height={16}
                  >
                    <path d="M13.95.879a3 3 0 0 0-4.243 0L1.293 9.293a1 1 0 0 0-.274.51l-1 5a1 1 0 0 0 1.177 1.177l5-1a1 1 0 0 0 .511-.273l1.16-1.16a1 1 0 0 0-1.414-1.414l-.946.946-3.232.646.646-3.232 8.2-8.2a1 1 0 0 1 1.414 0l1.172 1.172a1 1 0 0 1 0 1.414l-.55.549a1 1 0 0 0 1.415 1.414l.55-.55a3 3 0 0 0 0-4.241L13.948.879ZM3.25 4.5a1.25 1.25 0 1 0 0-2.5 1.25 1.25 0 0 0 0 2.5Zm11.474 6.029-1.521-.752-.752-1.521c-.168-.341-.73-.341-.896 0l-.752 1.52-1.521.753a.498.498 0 0 0 0 .896l1.52.752.753 1.52a.5.5 0 0 0 .896 0l.752-1.52 1.52-.752a.498.498 0 0 0 0-.896Z" />
                  </svg>
                  <span>Design</span>
                </button>
              </div>
            </div>

            {/* Carousel */}
            <div 
              className="relative mx-auto max-w-3xl"
              onMouseEnter={() => setIsAutoPlaying(false)}
              onMouseLeave={() => setIsAutoPlaying(true)}
            >
              {/* Cards Container */}
              <div className="overflow-hidden rounded-2xl">
                <div 
                  className="flex transition-transform duration-700 ease-out"
                  style={{ transform: `translateX(-${currentIndex * 100}%)` }}
                >
                  {filteredTestimonials.map((testimonial, index) => (
                    <div key={index} className="w-full flex-shrink-0 px-4">
                      <TestimonialCard testimonial={testimonial} />
                    </div>
                  ))}
                </div>
              </div>

              {/* Navigation Buttons */}
              <button
                onClick={prevSlide}
                className="absolute left-0 top-1/2 -translate-y-1/2 -translate-x-4 rounded-full bg-blue-600/90 p-2 text-white backdrop-blur-xs transition-colors hover:bg-blue-500 md:-translate-x-6"
                aria-label="Previous testimonial"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <path d="M15 18l-6-6 6-6" />
                </svg>
              </button>
              <button
                onClick={nextSlide}
                className="absolute right-0 top-1/2 -translate-y-1/2 translate-x-4 rounded-full bg-blue-600/90 p-2 text-white backdrop-blur-xs transition-colors hover:bg-blue-500 md:translate-x-6"
                aria-label="Next testimonial"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  width="20"
                  height="20"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                >
                  <path d="M9 18l6-6-6-6" />
                </svg>
              </button>

              {/* Dots Indicator */}
              <div className="mt-8 flex justify-center gap-2">
                {filteredTestimonials.map((_, index) => (
                  <button
                    key={index}
                    onClick={() => setCurrentIndex(index)}
                    className={`h-2 rounded-full transition-all ${
                      index === currentIndex
                        ? "w-6 bg-blue-500"
                        : "w-2 bg-blue-800/50 hover:bg-blue-600"
                    }`}
                    aria-label={`Go to testimonial ${index + 1}`}
                  />
                ))}
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
}

function TestimonialCard({
  testimonial,
}: {
  testimonial: {
    img: StaticImageData;
    clientImg: StaticImageData;
    name: string;
    company: string;
    content: string;
    categories: number[];
  };
}) {
  // Get first letter of the name
  const getInitial = (name: string) => {
    return name.charAt(0).toUpperCase();
  };

  return (
    <article className="relative rounded-2xl bg-gradient-to-br from-blue-950/90 via-blue-900/80 to-blue-950/90 p-6 backdrop-blur-sm before:pointer-events-none before:absolute before:inset-0 before:rounded-[inherit] before:border before:border-blue-500/20 before:[background:linear-gradient(to_right,#2563eb33,#3b82f666,#2563eb33)_border-box] before:[mask-composite:exclude_!important] before:[mask:linear-gradient(white_0_0)_padding-box,_linear-gradient(white_0_0)] md:p-8">
      <div className="flex flex-col gap-5">
        {/* Testimonial Content */}
        <p className="text-base text-blue-100 before:content-['“'] before:text-blue-400 before:text-xl after:content-['”'] after:text-blue-400 after:text-xl md:text-lg leading-relaxed">
          {testimonial.content}
        </p>
        
        {/* Person Initial and Name - Image replaced with initial */}
        <div className="flex items-center gap-3">
          {/* Initial Circle instead of image */}
          <div className="shrink-0">
            <div className="w-11 h-11 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-semibold text-lg ring-2 ring-blue-400/50">
              {getInitial(testimonial.name)}
            </div>
          </div>
          <div className="text-sm font-medium md:text-base">
            <span className="text-white">{testimonial.name}</span>
            <span className="text-blue-400"> - </span>
            <span className="text-blue-300">
              {testimonial.company}
            </span>
          </div>
        </div>
      </div>
    </article>
  );
}